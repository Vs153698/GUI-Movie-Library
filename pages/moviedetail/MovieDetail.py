...
class MovieDetail:
    ...
    def __init__(self,window,bg_color,imdbID=None,movies=[],relief=tk.SUNKEN, side=tk.LEFT):
        self.frame = tk.Frame(master=window,name='movieDetail',relief=relief,bg=bg_color)
        self.side = side
        self.imdbID = imdbID
        self.movies = movies
        self.add_frame()
    ...
    def add_page_title(self,title):
        if self.imdbID != None:
            lbl = tk.Label(master=self.frame, text=self.movie['Title'], height=3,
